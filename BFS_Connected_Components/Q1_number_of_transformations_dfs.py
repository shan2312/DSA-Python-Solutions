...
def get_min_operations_to_target(
    start_num, target_num, additive_nums, multiplicative_nums
):
    ...
    def get_min_operations_to_target_helper(current_num):
        ...
        for additive_num in additive_nums:
            ...
            count_if_addition = min(
                count_if_addition, 1 + get_min_operations_to_target_helper(new_num)
            )
        ...
        for multiplicative_num in multiplicative_nums:
            ...
            count_if_multiplication = min(
                count_if_multiplication,
                1 + get_min_operations_to_target_helper(new_num),
            )

        cached_min_operations[current_num] = min(
            count_if_addition, count_if_multiplication
        )
        return cached_min_operations[current_num]
