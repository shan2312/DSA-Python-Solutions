...
class Solution:
    ...
    def add_neighbors_to_queue(self, x, y, queue, visited_hashmap, num_moves):
        for neighbor in self.get_neighbors(x, y):
            if neighbor in visited_hashmap:
                continue
    ...
    def minKnightMoves(self, x: int, y: int) -> int:
        ...
        while True:
            (
                curr_origin_x,
                curr_origin_y,
                num_moves_so_far_from_origin,
            ) = origin_queue.popleft()
            (
                curr_target_x,
                curr_target_y,
                num_moves_so_far_from_target,
            ) = target_queue.popleft()

            if (curr_origin_x, curr_origin_y) in target_visited_hashmap:
                return (
                    num_moves_so_far_from_origin
                    + target_visited_hashmap[(curr_origin_x, curr_origin_y)]
                )

            if (curr_target_x, curr_target_y) in origin_visited_hashmap:
                return (
                    num_moves_so_far_from_target
                    + origin_visited_hashmap[(curr_target_x, curr_target_y)]
                )

            self.add_neighbors_to_queue(
                curr_origin_x,
                curr_origin_y,
                origin_queue,
                origin_visited_hashmap,
                num_moves_so_far_from_origin,
            )
