...
class Solution:
    ...
    def get_neighbors(self, word_list):
        for index, letter in enumerate(word_list):
            for alphabet in string.ascii_lowercase:
                if alphabet == letter: continue
                yield "".join(word_list[:index] + [alphabet] + word_list[(index + 1):])
    ...
    def findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        ...
        while queue:
            if last_iter: return transformations
            visited_set = set()
            for _ in range(len(queue)):
                current_word, transformation = queue.popleft()
                

                for neighbor in self.get_neighbors(list(current_word)):        
                    if neighbor == endWord:
                        transformations.append(transformation + [endWord])
                        last_iter = True

                    if neighbor not in word_set: continue
                    queue.append((neighbor, transformation + [neighbor]))
                    visited_set.add(neighbor)
            for word in visited_set:
                if word in word_set:
